...
class Modal extends React.Component {
  constructor(props) {
    ...
    this.handleResponseAddItems = this.handleResponseAddItems.bind(this);
    this.handleResponseAddDiscounts = this.handleResponseAddDiscounts.bind(this);
  }

  handleResponseAddItems(resp) {
  }
  ...
  addItemFunction() {
    ...
    console.log('reqBodyaddItems ->', reqBodyAddItems);
    MundipaggRecurrencyConnector('POST', `subscriptions/${subsId}/items`, reqBodyAddItems)
      .then(resp => (this.handleResponseAddItems(resp)));
  }

  handleResponseAddDiscounts(resp) {
    console.log('handle response add discounts ->', resp.data);
  }
  ...
  addDiscountFunction() {
    ...
    console.log('reqBodyaddDiscounts ->', reqBodyAddDiscounts);
    MundipaggRecurrencyConnector('POST', `subscriptions/${subsId}/discounts`, reqBodyAddDiscounts)
      .then(resp => (this.handleResponseAddDiscounts(resp)));
  }

  handleChange(event) {
    console.log('state handle change', this.state)
  ...
  itemObj(items, index) {
    ...
    itemValue = 'R$' + items.pricing_scheme.price / 100;
          ...
          <div className={containerClass}>
            <div className={styles.modalGrid}>
              <div className={styles.modalHeader}>
                ...
                <div className={styles.modalPlanData}>
                  <p>Começo em: <span>{this.props.input[this.props.inputIndex].current_cycle.start_at.substring(0, 10)}</span></p>
                </div>
                <div className={styles.modalPlanData}>
                  <p>Término em: <span>
                ...
                <div className={styles.modalPlanData}>
                  <p>Próxima cobrança: <span>{this.props.input[this.props.inputIndex].current_cycle.billing_at.substring(0, 10)}</span></p>
                </div>
                <div className={styles.modalPlanData}>
                  <p>Payment Method: <span>{this.props.input[this.props.inputIndex].payment_method}</span></p>
                </div>
              </div>
...
class SignatureInfos extends React.Component {
  ...
  populateSubscriptions(subsInfos, index, self) {
    ...
    for (i = 0; i < subsInfos.items.length; i++) {
      amount = amount + parseInt(subsInfos.items[i].pricing_scheme.price);
    }
    for (j = 0; j < subsInfos.discounts.length; j++) {
      ...
      return (
        <div className={styles.signatureInfos}>
          <div className={styles.row} ref='modalDiv'>
            {(this.state.isModalOpen ? <Modal amount={this.state.amount} inputIndex={this.state.subscriptionIndex} input={this.state.subscriptions} isModalOpen={this.state.isModalOpen} /> : null)}
          </div>
